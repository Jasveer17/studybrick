...
const ProfilePage = () => {
    ...
    const cardClass = `rounded-2xl border transition-[border-color,box-shadow] duration-150 hover:shadow-lg ${isDark ? 'bg-slate-800/80 border-slate-700 hover:border-slate-600' : 'bg-white border-slate-200 hover:border-indigo-200 hover:shadow-indigo-100/50'}`;
        ...
        <div className="max-w-4xl mx-auto space-y-6 fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className={`${cardClass} p-6`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                        <span className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors duration-150 hover:opacity-80 ${isDark ? 'bg-amber-500/20 text-amber-300' : 'bg-amber-100 text-amber-700'}`}>
                            🔥 7 Day Streak
                        </span>
                        <span className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors duration-150 hover:opacity-80 ${isDark ? 'bg-indigo-500/20 text-indigo-300' : 'bg-indigo-100 text-indigo-700'}`}>
                            📚 First Quiz
                        </span>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Days Remaining */}
                <div className={`${cardClass} p-6 ${isExpired ? isDark ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200' : isExpiringSoon ? isDark ? 'bg-amber-500/10 border-amber-500/30' : 'bg-amber-50 border-amber-200' : ''}`}>
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {user.allowedSubjects.map(subject => (
                                    <span
                                        key={subject}
                                        className={`px-4 py-2 rounded-xl text-sm font-semibold capitalize transition-colors duration-150 hover:opacity-80 ${isDark ? 'bg-violet-500/20 text-violet-300' : 'bg-violet-100 text-violet-700'}`}
                                    >
                                        {subject}
                                    </span>
                                ))}
            ...
            {/* Account Info */}
            <div className={`${cardClass} p-4 ${isDark ? 'bg-slate-800/50' : 'bg-slate-50/50'}`}>
